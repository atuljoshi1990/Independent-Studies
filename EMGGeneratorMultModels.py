...
try:
    ...
    for t in range(leng):
        ...
        outputLabel = numpy.array([Y])
        for v in range(u):
            X.append(dataset[t][v])
        inputData = numpy.array([X])
        model = Sequential()
        # Set the number of nurons and the number of inputs for the input layer.
        model.add(Dense(12, input_dim=u, activation='relu'))
        model.add(Dense(u, activation='relu'))
        model.add(Dense(1, activation='sigmoid'))
        # compile model
        model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])
        # fit the model
        model.fit(inputData, outputLabel, epochs=150, batch_size=10, verbose=0)
        modelList.append(model)
    rounded = []
    for i in range(leng):
        Q = []
        for j in range(u):
            Q.append(dataset[i][j])
        inpuData = numpy.array([Q])
        for k in range(len(modelList)):
            modell = modelList[k]
            prediction = modell.predict(inpuData)
            roundedTemp = [round(x[0]) for x in prediction]
            if roundedTemp[0] > 0.7:
                print(roundedTemp[0])
                rounded.append(roundedTemp[0])
                break;
